    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                # from_which_layer.append((torch.ones(size=(len(b),)) * i).to(torch.device('cuda:0'))) # Debug: Runtime Error Move to GPU
            ...
            # fg_mask_inboxes = fg_mask_inboxes.to(torch.device('cuda:0')) # Debug for runtime error indices should either be on CPU or on the same device as the indexed tensor (CPU)
            # fg_mask_inboxes = fg_mask_inboxes.to(targets.device) # Debug for runtime error indices should either be on CPU or on the same device as the indexed tensor (CPU)
...
class ComputeLossBinOTA:
    ...
    def find_3_positive(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            # Append
            a = t[:, 6].long()  # anchor indices
            indices.append((b, a, gj, gi))  # image, anchor, grid indices  -  Uncomment this line
            # indices.append((b, a, gj.clamp_(0, gain[3] - 1), gi.clamp_(0, gain[2] - 1)))  # image, anchor, grid indices - comment this line
            indices.append((b, a, gj.clamp_(0, gain[3] - 1), gi.clamp_(0, gain[2] - 1)))  # image, anchor, grid indices
            anch.append(anchors[a])  # anchors
